...
const registerUser = async (req, res) => {
  ...
  try {
    ...
    // ---- Admin (Barber) Register — DIRECT APPROVAL BYPASSED ----
    if (role === 'admin') {
      if (!shopCode) {
        return res.status(400).json({ message: 'Shop code is required for admin' });
      }
      ...
      // Password ko manually hash kar rahe hain kyunki ye seedha User model me ja raha hai
      const salt = await bcrypt.genSalt(10);
      const hashedPassword = await bcrypt.hash(password, salt);

      // ✅ CHANGE: AdminRequest ki jagah ab seedha direct 'User' table me record banega
      ...
      // ✅ CHANGE: Response me token bhej rahe hain taaki login automatic ho jaye register hote hi
      return res.status(201).json({ 
        id: barber.id,
        name: barber.name,
        email: barber.email,
        ...
        message: 'Barber registered and approved successfully!' 
      });
    }
